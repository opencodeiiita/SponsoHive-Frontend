...
const CampaignAutomation: React.FC = () => {
  ...
  const insertPlaceholder = (placeholder: string) => {
    ...
    const newValue =
      content.slice(0, selectionStart) + placeholder + content.slice(selectionEnd);
    setContent(newValue);
    setTimeout(() => {
      textarea.focus();
      textarea.selectionStart = textarea.selectionEnd = selectionStart + placeholder.length;
    }, 0);
  };
  ...
  const onFinish = ({ templateName }: { templateName: string }) => {
    if (!validatePlaceholders(content)) {
      Modal.error({ title: 'Invalid Placeholders', content: 'Use predefined placeholders only.' });
      return;
    }
    const placeholders = extractPlaceholders(content);
    setTemplates((prev) => [
  ...
  const deleteTemplate = (key: string) => setTemplates((prev) => prev.filter((item) => item.key !== key));

  const viewTemplate = (key: string) => {
    const template = templates.find((t) => t.key === key);
    if (template) {
      setSelectedTemplateContent(generatePreview(template.name));
      setIsViewModalOpen(true);
    }
  };
  ...
  const columns = [
    ...
    { title: 'Placeholders', dataIndex: 'placeholders', render: (placeholders: string[]) => placeholders?.join(', ') },
    {
      title: 'Actions',
      key: 'actions',
      render: (_: any, record: Template) => (
        <>
          <Button type="link" onClick={() => viewTemplate(record.key)}>View</Button>
          <Button type="link" onClick={() => deleteTemplate(record.key)}>Delete</Button>
        </>
      ),
    },
  ];
        ...
        <Form layout="vertical" onFinish={onFinish}>
          ...
          <Form.Item label="Content">
            ...
            <div className="placeholders">
              {predefinedPlaceholders.map((placeholder) => (
                <Button key={placeholder} onClick={() => insertPlaceholder(placeholder)}>
                  {placeholder}
                </Button>
              ))}
            </div>
