...
import { emailCampaignData, campaignSummary } from '../../../utils/dummyEmailData.ts';
...
const CampaignAutomation: React.FC = () => {
  ...
  const generatePreview = (content: string): string => {
    return content.replace(/{[a-zA-Z0-9]+}/g, (match) =>
      (dummyData as Record<string, string>)[match.slice(1, -1)] || match
    );
  };
      ...
      <section className="dashboard bg-gray-900 p-6 rounded-lg shadow-lg mb-8">
        ...
        <Row gutter={[16, 16]} className="mb-6">
          {Object.entries(campaignSummary).map(([key, value]) => (
            <Col span={6} key={key}>
              <Card>
                <Statistic title={`Total ${key}`} value={value} />
              </Card>
            </Col>
          ))}
        </Row>
